...
import { ReferenceAdminSidebar } from "@/components/portal/reference/ReferenceAdminSidebar";
...
export function AdminLayout() {
  const { isAdmin, isLoading } = usePortalAuth();
  ...
  return (
    <ReferenceShell>
      <ReferenceAdminSidebar className="hidden lg:flex" />

      <Sheet open={mobileNavOpen} onOpenChange={setMobileNavOpen}>
        <SheetContent side="left" className="w-[220px] border-[#E2E0DB] p-0 lg:hidden">
          <ReferenceAdminSidebar onNavigate={() => setMobileNavOpen(false)} />
        </SheetContent>
      </Sheet>
